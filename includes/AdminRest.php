...
class BeThemeSmartSearch_Admin_REST {
    ...
    public function get_settings() {
        ...
        return rest_ensure_response(array(
            'options' => $options,
            'defaults' => BeThemeSmartSearch_Helpers::get_default_options(),
            'pages' => $page_options,
        ));
    }
    ...
    public function update_settings(WP_REST_Request $request) {
        ...
        $options = BeThemeSmartSearch_Helpers::sanitize_options(is_array($payload) ? $payload : array());
    ...
    public function reset_settings() {
        update_option(BETHEME_SMART_SEARCH_OPTION_NAME, BeThemeSmartSearch_Helpers::get_default_options(), false);
    ...
    public function get_analytics(WP_REST_Request $request) {
        ...
        $top_no_results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT
                    query,
                    COUNT(*) as hits,
    ...
    public function test_query(WP_REST_Request $request) {
        ...
        $options = BeThemeSmartSearch_Helpers::get_options();
        $variants = BeThemeSmartSearch_Helpers::build_query_variants($q);
        if (empty($variants)) {
            $variants = array($q);
        }
        ...
        if (class_exists('WooCommerce')) {
            ...
            $debug['meta_query'] = array(
                'clauses' => max(0, count($meta_query) - 1),
                'variants' => $variants,
            );
            ...
            $debug['warning'] = 'WooCommerce не активен.';
        }
